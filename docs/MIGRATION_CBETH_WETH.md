...
- [ ] 증액 전개: 게이트(HL 증거금 ≥ $240) 충족 시 위 "증액 전개" 순서 0~6 실행 후 보고.
  미충족이면 이 항목은 건드리지 말고 [SILENT] 대기. **7-20 22:00까지도 미도착이면
  "Arbitrum 정체 $185.58 — HL Deposit 클릭 필요" 리마인드를 1회만 보고**하고
  (보고 후 이 줄에 리마인드 완료 표기) 계속 대기.
- [ ] 전환 후 24h 관측: 수수료 적재·비율 변동성·재헤지 빈도 확인, MJ에게 결과 보고
  (증액 전개가 실행됐다면 그 시점부터 24h 재기산)
