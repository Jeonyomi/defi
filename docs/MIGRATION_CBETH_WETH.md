...
- [x] 전액 전개 — 2026-07-20 22:35 (별도 세션 기록 보완: HL 추가입금 후 대기 USDC $207
  포함 LP $602 재전개, 숏 0.3206 ETH, lev 2.45x — MJ 승인 하드리밋 3x 내.
  22:25 mint STF(승인) 실패 1회 → 22:35 재시도 성공. events 테이블 실측)
- [x] 전환 후 24h 관측 + 보고 — 2026-07-21 19:30 완료 (전액 전개 후 20.7h, 스냅샷 125건 실측:
  ① 수수료 적재 ≈ $0.0001 (simulate_collect 온체인 실측 — 풀 fee 0.007%·비율페어 거래량
  저조로 사실상 0, 현 구조의 수익 동력은 펀딩), ② 비율 1.135247~1.135273 진폭 0.0023%
  (±2% 레인지 대비 여유 막대, rerange 위험 없음), ③ 재헤지 0회 — HL 체결 0건, 델타 갭
  최대 $0.01 (full-delta 구조상 갭 자체가 안 생김), ④ equity $845.36→$845.55 (+$0.19,
  펀딩 ≈ +$0.16) — ETH +3.7% 랠리에서 upnl -$22 ↔ LP +$22 상쇄로 델타중립 실증.
  총자산 $849.7(지갑 USDC $4.15 포함) = 전환 전 $259.2 + 입금 $590.6과 일치(누수 없음).
  잔여 리스크: lev 2.79x(한도 3x), ETH 추가 상승 시 근접 — HL 단독 청산가 ≈ $2,585(+33%).
  7-21 19:06 봇 재기동 후 사이클 정상, ERROR 0건)

## 재배치 델타 규율 (2026-07-20 사후 보완 — MJ 지적 반영)

증액 전개에서 발견된 절차 결함: 스왑(19:05, USDC→WETH ~$290)과 헤지 증액(19:09,
봇 재기동 후 자동진입)이 분리되어 **~$290 롱이 약 4분 무헤지 노출**됐다. 결과적으로
손실은 없었지만(그 구간 가격 변동 미미), 절차상 재발 방지를 코드로 강제한다:

1. **수동 절차** — `migrate_cbeth_weth.py`의 모든 tx 서브커맨드(close/close-cbeth/
   swap-to-weth/swap-to-cbeth)는 끝에서 `sync_hedge()`를 자동 호출: 총델타(지갑+양풀
   LP ETH-eq)를 실측해 갭 ≥ $15면 즉시 숏을 수렴시킨다. 수동 개입 후 갭 정리는
   `sync-hedge` 단독 실행. **델타를 바꾸는 tx와 헤지 조정은 같은 프로세스에서 붙어
   실행된다** — 봇 재기동을 헤지 조정 수단으로 쓰지 않는다.
2. **봇 재배치 경로** — `_do_rerange`가 모든 경로에서 헤지를 동기화: mint 실패 시
   지갑 실측 델타로 즉시 재헤지 후 오류 전파, 포지션 조회 지연 시 추정치로 즉시
   동기화(다음 사이클 드리프트 분기가 실측 보정). full_delta 페어는 청산→스왑→mint
   내내 총델타 불변이라 자연 안전하고, 이 보강은 usd 페어·실패 경로를 커버한다.
3. **검증** — tests/verify_rebalancer_pair.py §7 (실패·지연 4케이스), sync-hedge
   라이브 스모크(갭 $0.0 → no-op 확인, 2026-07-20).

예외 허용: 예기치 못한 상황(RPC 전면 장애, HL 다운)으로 동기화가 불가능하면 노출
시간 최소화를 우선하되, 수동 `sync-hedge` 재시도를 최우선 복구 절차로 한다.
